...
class GoalObjectsListener(object):
    ...
    def gazebo_co_callback(self, data):
        ...
        # newdict = {}
        # print(newdict)
        if (time - self.prev_time).total_seconds() > self.time_interval:
            self.prev_time = time
            for i in range(len(names)):
                if i!= heightmap_index:
                    ...
                    if self.prev_twists[i] != twists[i]:
                        newdict = {i: {"pose": {"position": {"x": round(positions[i].position.x, 2),
                                                             "y": round(positions[i].position.y, 2),
                                                             "z": round(positions[i].position.z, 2)},
                                                "orientation": {"x": round(positions[i].orientation.x, 2),
                                                                "y": round(positions[i].orientation.y, 2),
                                                                "z": round(positions[i].orientation.z, 2),
                                                                "w": round(positions[i].orientation.w, 2)}},
                                   "twist": {"angular": {"x": round(twists[i].angular.x, 2),
                                                         "y": round(twists[i].angular.y, 2),
                                                         "z": round(twists[i].angular.z, 2)},
                                             "linear": {"x": round(twists[i].linear.x, 2),
                                                        "y": round(twists[i].linear.y, 2),
                                                        "z": round(twists[i].linear.z, 2)}}}}
